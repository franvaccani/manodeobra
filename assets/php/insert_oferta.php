...
//conectarme tabla usuarios

//consultar plan activo
//if(plan = 1) 1 es gratis
//Cuantas publicaciones tiene este mes
// if(publi_activas < 10) 10 es el limite mensual
    ...
    if($con){
        $sql_update_img = "UPDATE ofertas_imagenes SET oferta_imgoferta='$ultimo_id', tmp_imgoferta = null WHERE tmp_imgoferta ='$token' and estado_imgoferta = 1 ";
        ...
        if($oferta=='true'){
        $sql_promo = "INSERT INTO promociones SET estado_promocion='1', desde_promocion='$desde_promo', hasta_promocion='$hasta_promo', titulo_promocion='$titulo_promo', detalle_promocion='$detalle_promo', tipo_promocion='$tipo_promo', valor_promocion='$valor_promo', oferta_promocion='$ultimo_id' ";
        $inserto_promo = $link->query($sql_promo);
        }else{
        //  echo 'entra sin oferta';
        }

        $buca_portada = $link->query("SELECT id_imgoferta FROM `ofertas_imagenes` WHERE `oferta_imgoferta` = $ultimo_id order by id_imgoferta asc limit 1 ");

        if($row=mysqli_fetch_array($buca_portada)){
        $id_foto = $row['id_imgoferta'];
        $updateo_princi = $link->query("UPDATE ofertas SET foto_oferta='$id_foto' WHERE id_oferta = $ultimo_id");
        }                                   
